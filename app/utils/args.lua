...
function _M:get_from_ngx_req()
    ...
    if "GET" == request_method then
        ...
        local is_body_transed, body, err = u_request:get_transform_post()
        args = u_request:get_transform_json(body)
    -- elseif "POST" == request_method then
    --     ngx.req.read_body()

    --     --判断是否是multipart/form-data类型的表单
    --     content_type = receive_headers["content-type"]
    --     if s_sub(content_type or "",1,20) == "multipart/form-data;" then   

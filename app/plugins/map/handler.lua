...
---> 当前插件具备一定特殊性，重写父类规则
--]]
function handler:_stop_check(rule, rule_passed, rule_failure_func)
    -- ngx.log(ngx.ERR, self.format("%s|%s|%s|%s",rule.name,rule_passed, self.utils.json.encode(rule_failure_func), self.utils.json.encode(rule.judge)))

    local continue = rule.handle.continue
    ...
    return not rule.handle.continue --handler.super._stop_check(self, rule, rule_passed)
end
